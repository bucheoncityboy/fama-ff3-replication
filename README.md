...
`appendix_output/table2_panel1_factor_summary.csv` 기준 핵심 값은 다음과 같다.
...
이 중 주식 측 핵심인 RM-RF, SMB, HML은 이전 비교 기준에서도 논문 값과 매우 가까운 수준을 유지했다.

### 4.2 Size / Value 효과

`appendix_output/table2_panel2_stock_mean_std.csv`를 보면:

- Small-High BM 셀이 **1.06%/월**로 가장 높은 축에 속한다.
- Big-Low BM은 **0.39%/월** 수준으로 더 낮다.
- 즉, **HiBM > LoBM**, **Small > Big** 패턴이 동시에 확인된다.

이건 논문이 주장한 가치 효과(HML)와 규모 효과(SMB)의 원자료 패턴 자체가 재현됐다는 의미다.

### 4.3 3요인 모형의 설명력

`appendix_output/table6_panel4_r2_se.csv`는 25개 주식 포트폴리오에 대한 3요인 모형의 `R² / s(e)`를 담는다.

평균 설명력은 약 **0.8992**다.

즉, 시장요인 하나만 쓰는 Table 4보다 3요인 모형이 훨씬 잘 맞는다. 이 결과는 “주식 수익률은 Mkt-RF 하나로는 부족하고 SMB·HML이 반드시 필요하다”는 논문 핵심을 직접 뒷받침한다.

### 4.4 5요인 모형의 해석

`appendix_output/table7a_panel6_r2_se.csv`는 5요인 모형의 `R² / s(e)`다.

- 주식 평균 R²: 약 **0.8999**
- 3요인 대비 개선폭: 약 **+0.07pp**

즉, TERM/DEF를 추가해도 **주식 측 설명력 증가는 거의 없다**. 이것이 바로 논문이 말한 “채권 요인은 주식 횡단면에 큰 추가 기여를 하지 않는다”는 해석과 맞닿는다.

### 4.5 RMO 결과

`appendix_output/table8a_rmo.csv`와 `appendix_output/table8a_*`를 기준으로 보면:

- `corr(RMO, SMB) ≈ 0`
- `corr(RMO, HML) ≈ 0`
- `corr(RMO, TERM) ≈ 0`
- `corr(RMO, DEF) ≈ 0`
- `corr(RMO, Mkt-RF) = 0.9091`

즉, RMO는 실제로 직교화된 시장요인으로 작동한다. 그리고 이 상태에서도 Table 8a의 평균 R²는 높은 수준을 유지해, 시장요인의 정제된 부분이 여전히 중요한 설명변수라는 점을 보여준다.

### 4.6 E/P · D/P 정렬 결과

`appendix_output/table11_ep_dp_long.csv` 기준 대표 결과:

- EP <= 0 포트폴리오: CAPM R² = **0.6593**, FF3F R² = **0.8341**
- EP High 포트폴리오: CAPM R² = **0.8088**, FF3F R² = **0.9129**
- DP = 0 포트폴리오: CAPM R² = **0.8181**, FF3F R² = **0.9293**
- DP High 포트폴리오: CAPM R² = **0.7049**, FF3F R² = **0.8064**

즉, E/P·D/P 정렬 포트폴리오에서도 **CAPM보다 FF3F가 더 높은 설명력**을 보인다.

### 4.7 절편과 공동검정

`appendix_output/table9a_stock_alphas.csv`는 개별 주식 포트폴리오의 alpha/t(alpha)를 보여준다.

`appendix_output/table9c_joint_tests.csv`는 모형별 공동절편 검정 결과다.

대표적으로:

- 모형 (ii) 단일 시장요인: F = **62.44**
- 모형 (iv) 3요인: F = **59.31**
- 모형 (v) 5요인: F = **1.74**, p = **0.0097**

해석상 완전히 “모든 알파가 0”이라고 말할 수는 없지만, 단순 모형에 비해 다요인 구조가 pricing error를 크게 줄인다는 점은 분명하다.

---

## 5. 제출용 시각화 결과

### Figure 1. 25개 포트폴리오 평균 초과수익률 히트맵

![Figure 1](appendix_output/submission_fig1_stock_mean_heatmap.png)

이 그림은 25개 Size×BE/ME 포트폴리오의 평균 초과수익률을 한눈에 보여준다. 오른쪽(High BM)으로 갈수록 수익률이 높아지는 값이 많고, 상단 Small 쪽이 하단 Big보다 높은 셀이 자주 나타난다. 즉, **가치 효과와 규모 효과가 동시에 시각적으로 확인**된다.

### Figure 2. 요인 프리미엄 평균

![Figure 2](appendix_output/submission_fig2_factor_premiums.png)

주식 3요인과 채권 2요인의 평균 크기를 비교하면 TERM과 DEF가 절대 크기로는 크지만, 주식 횡단면 설명의 핵심은 여전히 SMB/HML 구조라는 점을 함께 볼 수 있다.

### Figure 3. 주식 포트폴리오 평균 설명력 비교

![Figure 3](appendix_output/submission_fig3_model_r2.png)

이 그림은 1요인, TERM/DEF 2요인, 3요인, 5요인, RMO+4요인 모형의 평균 R²를 비교한다. 핵심 메시지는 명확하다.

- 1요인 → 3요인: 큰 점프
- 3요인 → 5요인: 거의 변화 없음

즉, **논문에서 말한 주식 측 핵심은 3요인 구조**라는 점을 그림 하나로 요약한다.

### Figure 4. E/P·D/P 포트폴리오의 CAPM vs FF3F 설명력

![Figure 4](appendix_output/submission_fig4_ep_dp_r2.png)

모든 막대에서 FF3F가 CAPM보다 같거나 더 높다. 이는 부가 정렬 포트폴리오에서도 3요인 모형이 단일 시장모형보다 더 강하다는 점을 보강한다.
